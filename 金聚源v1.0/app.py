...
def sales_order_plan_activation_blocker(conn, order_id):
    ...
    if status != '差异已确认':
        status = compare_contract_repayment_plans(conn, contract['id']).get('status') or status
    if status not in ('已通过', '差异已确认'):
        return f'还款计划比对状态为{status}，请先完成厂家分期表比对或确认差异'
    return None
...
def sales_order_planning_contract(order_id):
    ...
    if request.method == 'PUT' and request.current_user['role'] == '财务':
        conn.close()
        return jsonify({'success': False, 'message': '客户分期由运营维护，财务请上传厂家分期表并完成比对'}), 403
    ...
    if request.method == 'GET' and not order['contract_id']:
        ...
        return jsonify({
            ...
            'comparison': {
                'status': '未生成',
                'message': '请先保存客户还款计划，再上传厂家分期表并完成比对',
            },
        })
    ...
    return jsonify({
        ...
        'factory_repayments': factory_rows,
        'comparison': comparison,
        'message': '客户还款计划已生成，请上传厂家分期表并完成比对',
    })
...
def add_contract():
    ...
    try:
        ...
        requested_contract_id = data.get('supplement_contract_id') or order['contract_id']
        ...
        if not existing_order_contract:
            c.execute("""
                SELECT id, vehicle_id, contract_status, delivery_status, contract_file
                FROM contracts
                WHERE sales_order_id=?
        ...
        start_date = normalize_date(data.get('start_date')) or normalize_date(order['payment_date']) or today
        try:
        ...
        if loan_periods <= 0:
            loan_periods = parse_period_count(order['lease_term'], factory_count or 12)
        total_price = parse_money(data.get('total_price'), parse_money(order['sale_total_price']))
        rent = parse_money(data.get('rent'), parse_money(order['vehicle_rent_amount']))
        ...
        if contract_type == '销售':
            ...
            deposit = parse_money(data.get('deposit'), 0)
            down_payment = parse_money(data.get('down_payment'), order_payment_amount or parse_money(order['car_purchase_amount']))
        ...
        rental_method = data.get('rental_method') or ('经营租赁' if contract_type == '租赁' else contract_type)
        company = (data.get('company') or order['receiving_company'] or '').strip()
