...
class FullFlowTestCase(unittest.TestCase):
    ...
    def test_finance_approval_center_blocks_installment_order_without_factory_plan(self):
        vehicle_id = self.create_vehicle({"guidance_price": 100000})
        vehicle = self.get_vehicle(vehicle_id)
        ...
        blocked_response = self.client.post(
            ...
            json={"comment": "试图绕过厂家分期"},
        )
        self.assertEqual(blocked_response.status_code, 400, blocked_response.get_json())
        self.assertIn("厂家还款计划未上传", blocked_response.get_json()["message"])
        self.logout()
        self.assertEqual(
